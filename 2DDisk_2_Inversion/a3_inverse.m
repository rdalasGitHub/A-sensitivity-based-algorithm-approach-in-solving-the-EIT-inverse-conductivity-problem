...
filename = sprintf('projLHSR2LHSR3_unoise0_CM2D_synthetic_d7_r1.txt');
LHStemp = importdata(filename, delimiterIn);
LHS = LHStemp(5:6);

%fits %does not change
R2slopefit = importdata('a5_R2slopep_4fit.txt');
...
%[sold, solr] = solve(r*R2slopef + R2yinterceptf == LHS(1).^(1/2.0487), r*R3slopef + R3yinterceptf == LHS(2).^(1/2));
[sold, solr] = solve(r*R2slopef == LHS(1).^(1/2.0487), r*R3slopef == LHS(2).^(1/2));
